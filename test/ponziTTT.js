...
contract('PonziTTT', function(accounts) {
    ...
    it("should register correctly", function() {
        ...
        return PonziTTT.new().then(function(instance) {
            ...
            web3.eth.sendTransaction({from: account_two, to: ponzi.address, value: web3.toWei(2, 'ether')});
        }).then(function(){
            return ponzi.isTrainee(account_two);
        }).then(function(res){
            assert.equal(res.valueOf(), true, "");
        }).then(function(){
            return ponzi.balanceOf(account_two);
        }).then(function(res){
            assert.equal(res.valueOf(), web3.toWei(2, 'ether'), "");
        })
    });
    ...
    it("should refund to trainee 4 if trainee_two did not finished", function() {
        var ponzi;
        var before, after;
        ...
        return PonziTTT.new([trainer, trainer_two], 4).then(function(instance) {
            ...
            return ponzi.checkProgress({from: trainee_two});
        }).then(function(res){
            assert.equal(res.valueOf(), 0, "");
        }).then(function() {
            return ponzi.autoRefund({from: trainer});
